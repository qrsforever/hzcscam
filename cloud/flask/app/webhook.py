...
async def _mqtt_rtmp_enable():# {{{
    ...
    try:
        ...
        info = g_mqtt_client.publish(
            qos=2,
            topic=f'cloud/{did}/camera/rtmp',
            payload=json.dumps({"rtmp_enable": False}))
        app.logger.info(f'publish {did} rtmp false: {info.rc}')
        ...
        if (jdata.get('enable')):
            ...
            app.logger.info(f'publish {did} {cid} video device: {info.rc}')

            info = g_mqtt_client.publish(
                qos=2,
                topic=f'cloud/{did}/camera/rtmp',
                payload=json.dumps({"rtmp_enable": True}))
            app.logger.info(f'publish {did} rtmp true: {info.rc}')
